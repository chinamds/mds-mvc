...
/**
 * Testing the MDSConfigurationFactoryBean to ensure it performs property
 * substitution in Spring XML configs (e.g. replacing ${mdsplus.home} with the
 * value from mds.cfg)
 * <P>
 ...
 * @author Tim Donohue
 */
public class MDSConfigurationFactoryBeanTest
    extends MDSAbstractKernelTest {
    ...
    public void init() {
        ...
        propertyFilePath = new MDSConfigurationService().getMDSHome(null) + File.separatorChar
    ...
    public void testGetBeanSettingFromConfigurationService() {
        ...
        // The bean's getProperty() method should return the same value as "testDynamicBean.property" in MDS's
     ...
     * NOTE: This test also fails in Commons Configuration v1 (e.g. in MDS 6.x). So, it's possible we may not be
    ...
    public void testReloadBeanSettingFromConfigurationService() throws ConfigurationException, InterruptedException {
        ...
        TestDynamicPropertyBean bean = getKernel().getServiceManager().getServiceByName("dynamicPropertyBean",
                                                                                        TestDynamicPropertyBean.class);
        assertNotNull("WeLoveMDS", bean.getProperty());
        ...
        assertEquals("WeLoveMDS", bean.getProperty());
