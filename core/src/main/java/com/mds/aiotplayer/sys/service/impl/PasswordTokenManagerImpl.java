...
public class PasswordTokenManagerImpl implements PasswordTokenManager {
    private final Logger log = LoggerFactory.getLogger(PasswordTokenManagerImpl.class);
    ...
    public String generateRecoveryToken(final User user) {
        if (user != null) {
            ...
            //return expirationTimeStamp + HelperFunctions.entryptPassword(expirationTimeStamp + tokenSource);
        }
        return null;
    }
    ...
    public boolean isRecoveryTokenValid(final User user, final String token) {
        if (user != null && token != null) {
            ...
            /*return expirationTime != null && expirationTime.after(new Date())
                    && HelperFunctions.validatePassword(tokenSource, tokenWithoutTimestamp);*/
        }
        return false;
    }
