...
class HibernateSearchTools {
   ...
   /* *//**
     * return a page result with lucene query  to search for a given term in all the indexed fields of a class
     *
     * @param searchTerm the term to search for
     * @param searchedEntity the class searched
     * @param sess the hibernate session
     ...
     * @throws ParseException
     *//*
    public static <E> Page<E> generateQuery(Page<E> page, String searchTerm, Class searchedEntity, Session sess, Analyzer defaultAnalyzer) throws ParseException {
        Query qry = null;
        ...
        if (searchTerm.equals("*")) {
            ...
            try {
                ...
                //Gets the last element of the previous page
                IndexSearcher searcher = new IndexSearcher(reader);
        		ScoreDoc lastSd = getLastScoreDoc(page.getPageNo(), page.getPageSize(), qry, searcher);
        		//Go through the last element to search for the elements on the next page
        		TopDocs tds = searcher.searchAfter(lastSd, qry, page.getPageSize());
        		page.setList(tds.scoreDocs);
        		tds.
    ...
    *//**
	 * Get the last 'scoredocs' based on the page number and paging size
	 * @param pageIndex
	 * @param pageSize
	 * @param query
	 * @param searcher
	 ...
	 * @throws IOException
	 *//*
	private static ScoreDoc getLastScoreDoc(int pageIndex, int pageSize, org.apache.lucene.search.Query query, IndexSearcher searcher) throws IOException {
		if(pageIndex==1)return null;//retrun null if first page
		int num = pageSize*(pageIndex-1);//retrieve preview page's record number
