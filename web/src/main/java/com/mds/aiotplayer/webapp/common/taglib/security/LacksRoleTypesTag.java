...
/**
 * Shiro LacksRoleTypes Tag.
 * 
 * @author John Lee
 */
public class LacksRoleTypesTag extends RoleTag  {
	...
	protected boolean showTagBody(String roleTypes) {
        ...
        if (user != null) {
			return user.getRoles().stream().allMatch(r->!ArrayUtils.contains(roleTypes.split(ROLE_TYPES_DELIMETER), r.getRoleType().toString()));
		} 
