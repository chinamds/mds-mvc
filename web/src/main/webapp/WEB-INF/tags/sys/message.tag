...
<%@ attribute name="content" type="java.lang.String" required="true" description="message content"%>
<%@ attribute name="type" type="java.lang.String" description="message type：info、success、warning、error、loading"%>
<script type="text/javascript">top.$.jBox.closeTip();</script>
<c:if test="${not empty content}">
	<c:if test="${not empty type}"><c:set var="ctype" value="${type}"/></c:if><c:if test="${empty type}"><c:set var="ctype" value="${fn:indexOf(content,'failure') eq -1?'success':'error'}"/></c:if>
