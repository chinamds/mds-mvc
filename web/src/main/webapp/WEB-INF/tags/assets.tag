...
<c:if test="${empty languageTag}"><c:set var="languageTag" value="${fns:getLanguageTag(pageContext.request)}"/></c:if>
<c:choose>
<c:when test="${empty languageTag or languageTag eq'en' or languageTag eq 'en_US' }">
<c:if test="${empty group}"><c:set var="group" value="main"/></c:if>
</c:when>
<c:otherwise>
	<c:choose>
		<c:when test="${empty group}">
			<c:set var="group" value="main_${languageTag}"/>
		</c:when>
		<c:otherwise>
			<c:set var="group" value="${group}_${languageTag}"/>
		</c:otherwise>
	</c:choose>
</c:otherwise>
</c:choose>

<%-- <c:set var="languageTag" value="${fns:getLanguageTag(pageContext.request)}"/> --%>
 <c:set var="xml">
<c:choose>
<c:when test="${empty languageTag or languageTag eq 'en' or languageTag eq 'en_US' }">
	<jsp:include page="/WEB-INF/wro.xml" flush="true" />
</c:when>
<c:otherwise>
	<jsp:include page="/WEB-INF/wro_${languageTag}.xml" flush="true" />
</c:otherwise>
</c:choose>
</c:set>
