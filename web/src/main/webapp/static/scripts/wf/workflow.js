<script type="text/javascript">
    $(function () {
    	initWorkflowTable("table", '${ctx}/services/api/workflows/organization/table', {q: $("#query").val()});
		...
		$("#delete").click(function(){
			...
			if (ids.length>0){
				top.$.mdsForm.confirm("<fmt:message key="table.message.deleteconfirm" />","<fmt:message key="albumList.album" />", {ok: function(){
					$.ajax({
		                url: '${ctx}/services/api/workflows/' + ids,
		                type: 'DELETE',
		                success: function () {
		...
		$("#btnExport").click(function(){
			top.$.mdsForm.confirm("<fmt:message key="message.export.confirm" />","<fmt:message key="exportform.title" />", {ok: function(){
				$.fileDownload("${ctx}/wf/workflows/export")
		...
		$("#btnImport").click(function(){	
			$.mdsForm.showFormDialog({
			     ...
			     postUrl: "${ctx}/wf/workflows/import",
			     isReadOnly: false,
			     isImportForm: true,
	...
	function actionFormatter(value) {
        return [
            '<a class="update" href="${ctx}/wf/workflowform?id=' + value + '" title="<fmt:message key="button.edit.tip"/>"><i class="fa fa-edit"></i></a>'
    ...
    function nameFormatter(value, row) {
   		var title = '<a href="${ctx}/wf/workflowform?id=' + row.id + '" class="btn btn-link no-padding">' + value + '</a>';
    ...
    window.actionEvents = {
        'click .remove': function (e, value, row) {
            ...
            if (confirmMessage(msgDelConfirm)) {
                $.ajax({
                    /*url: '${ctx}/workflowform?delete=&id=' + row.id,
                    type: 'post',*/
                	url: '${ctx}/services/api/workflows/' + row.id,
	                type: 'DELETE',
                    success: function () {
                    	alert('<fmt:message key="album.deleted"/>');
                    	$("#table").bootstrapTable('refresh');
    ...
    function initWorkflowTable(tableId, url, searchformData, options, columns) {
		var defaults = {
				 url: url,   
